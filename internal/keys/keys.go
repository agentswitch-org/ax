...
var Defs = []Def{
	...
	{Jump, "search", "quick-switch: type to fuzzy-find any session and open it (ignores filters)", []string{"'"}},
	...
	{DetachWin, "session", "close the viewer window; the session keeps running in the background", []string{"w"}},
	{OpenWin, "session", "reopen a window for the selected session and attach it", []string{"o"}},
